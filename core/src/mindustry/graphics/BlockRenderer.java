...
public class BlockRenderer {
    //TODO cracks take up far to much space, so I had to limit it to 7. this means larger blocks
    // won't have cracks - draw tiling mirrored stuff instead?
    public static final int crackRegions = 8, maxCrackSize = 7;
    public static boolean drawQuadtreeDebug = false;
    public static final Color shadowColor = new Color(0, 0, 0, 0.71f), blendShadowColor =
            Color.white.cpy().lerp(Color.black, shadowColor.a);
    ...
    public BlockRenderer() {
        ...
        Events.on(TilePreChangeEvent.class, event -> {
            if (blockTree == null || floorTree == null) {
                return;
            }
            ...
            if (indexFloor(event.tile)) {
                floorTree.remove(event.tile);
            }
        });

        Events.on(TileChangeEvent.class, event -> {
            boolean visible =
                    event.tile.build == null || !event.tile.build.inFogTo(Vars.player.team());
    ...
    public void updateDarkness() {
        ...
        if (state.rules.limitMapArea) {
            ...
            Fill.crect(state.rules.limitX, state.rules.limitY, state.rules.limitWidth,
                    state.rules.limitHeight);
        }

        for (Tile tile : world.tiles) {
            //skip lighting outside rect
            if (state.rules.limitMapArea && !Rect.contains(state.rules.limitX, state.rules.limitY
                    , state.rules.limitWidth - 1, state.rules.limitHeight - 1, tile.x, tile.y)) {
                continue;
            }
    ...
    public void removeFloorIndex(Tile tile) {
        if (indexFloor(tile)) {
            floorTree.remove(tile);
        }
    }

    public void addFloorIndex(Tile tile) {
        if (indexFloor(tile)) {
            floorTree.insert(tile);
        }
    }
    ...
    boolean indexFloor(Tile tile) {
        return tile.block() == Blocks.air && tile.floor().emitLight && world.getDarkness(tile.x,
                tile.y) < 3;
    }
    ...
    public void drawDestroyed() {
        ...
        if (brokenFade > 0.001f) {
            for (BlockPlan block : player.team().data().plans) {
                ...
                Draw.rect(b.fullIcon, block.x * tilesize + b.offset,
                        block.y * tilesize + b.offset, b.rotate ? block.rotation * 90 : 0f);
            }
            Draw.reset();
        }
    }
    ...
    public void drawShadows() {
        ...
        float u = (x - camera.width / 2f) / ww, v = (y - camera.height / 2f) / wh, u2 =
                (x + camera.width / 2f) / ww, v2 = (y + camera.height / 2f) / wh;
        ...
        blockTree.intersect(bounds, tile -> {
            ...
            if (tile.build != null && tile.build.power != null && tile.build.power.links.size > 0) {
                for (Building other : tile.build.getPowerConnections(outArray2)) {
                    if (other.block instanceof PowerNode && procLinks.add(other.id)) { //TODO
                        // need a generic way to render connections!
    ...
    void drawTree(QuadTree<Tile> tree) {
        ...
        for (var tile : tree.objects) {
            var block = tile.block();
            Tmp.r1.setCentered(tile.worldx() + block.offset, tile.worldy() + block.offset,
                    block.clipSize, block.clipSize);
            Lines.rect(Tmp.r1);
        }
    ...
    public void drawBlocks() {
        ...
        for (int i = 0; i < tileview.size; i++) {
            ...
            if (block != Blocks.air && (visible || build.wasVisible)) {
                ...
                //TODO here is the question: should buildings you lost sight of remain rendered?
                // if so, how should this information be stored?
        ...
        if (renderer.lights.enabled()) {
            ...
            for (int i = 0; i < lightview.size; i++) {
                ...
                } else if (tile.floor().emitLight && tile.block() == Blocks.air) { //only draw
                    // floor light under non-solid blocks
    ...
    public void updateShadow(Building build) {
        ...
        int size = build.block.size, of = build.block.sizeOffset, tx = build.tile.x, ty =
                build.tile.y;
    ...
    static class BlockQuadtree extends QuadTree<Tile> {
        ...
        public void hitbox(Tile tile) {
            var block = tile.block();
            tmp.setCentered(tile.worldx() + block.offset, tile.worldy() + block.offset,
                    block.clipSize, block.clipSize);
        }
