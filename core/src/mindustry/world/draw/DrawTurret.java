...
public class DrawTurret extends DrawBlock {
    ...
    public void draw(Building build) {
        ...
        if(randomizer.worldState.options.getRandomizeBlocksSize()){
            ...
            drawTurretRandomizer(turret, tb);
            drawHeatRandomizer(turret, tb);
        } else {
        ...
        if (parts.size > 0) {
            ...
            var params = DrawPart.params.set(build.warmup(), 1f - progress, 1f - progress,
                    tb.heat, tb.curRecoil, tb.charge, tb.x + tb.recoilOffset.x,
                    tb.y + tb.recoilOffset.y, tb.rotation);

            for (var part : parts) {
                params.setRecoil(part.recoilIndex >= 0 && tb.curRecoils != null ?
                        tb.curRecoils[part.recoilIndex] : tb.curRecoil);
                part.draw(params);
            }
        }
    }
    ...
    public TextureRegion[] icons(Block block) {
        return top.found() ? new TextureRegion[]{base, preview, top} : new TextureRegion[]{base,
                preview};
    }
}
