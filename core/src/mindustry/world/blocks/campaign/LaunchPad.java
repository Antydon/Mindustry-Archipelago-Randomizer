...
public class LaunchPad extends Block {
    ...
    public void setBars() {
        ...
        addBar("progress", (LaunchPadBuild build) -> new Bar(() -> Core.bundle.get("bar" +
                ".launchcooldown"), () -> Pal.ammo,
    ...
    public class LaunchPadBuild extends Building {
        ...
        public void draw() {
            ...
            if (lightRegion.found()) {
                ...
                float progress = Math.min((float) items.total() / itemCapacity,
                ...
                for (int i = 0; i < 4; i++) {
                    for (int j = 0; j < steps; j++) {
                        ...
                        Draw.rect(lightRegion, x + Geometry.d8edge(i).x * offset,
                                y + Geometry.d8edge(i).y * offset, i * 90);
                    }
                }
        ...
        public void display(Table table) {
            ...
            table.row();
            table.label(() -> {
                Sector dest = state.rules.sector == null ? null :
                        state.rules.sector.info.getRealDestination();

                return Core.bundle.format("launch.destination", dest == null || !dest.hasBase() ?
                        Core.bundle.get("sectors.nonelaunch") : "[accent]" + dest.name());
    ...
    static abstract class LaunchPayloadComp implements Drawc, Timedc, Teamc {
        ...
        public void draw() {
            ...
            TextureRegion region =
                    blockOn() instanceof mindustry.world.blocks.campaign.LaunchPad p ?
                            p.podRegion : Core.atlas.find("launchpod");
            scale *= region.scl();
