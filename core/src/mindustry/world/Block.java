...
public class Block extends UnlockableContent implements Senseable {
    ...
    /**
     * Used by certain power blocks (nodes) to flag as non-consuming of power. True by default,
     * even if this block has no power.
     */
    ...
    /**
     * higher numbers increase liquid output speed; TODO remove and replace with better liquids
     * system
     */
    ...
    /**
     * if rotate = false and this is true, rotation will be locked at 0 when placing (default);
     * advanced use only
     */
    ...
    /**
     * if true, this logic-related block can only be used with privileged processors (or is one
     * itself)
     */
    ...
    /**
     * whether to automatically reset enabled status after a logic block has not interacted for a
     * while.
     */
    ...
    /**
     * Health per square tile that this block occupies; essentially, this is multiplied by size *
     * size. Overridden if health is > 0. If <0, the default is 40.
     */
    ...
    /**
     * The block group. Unless {@link #canReplace} is overridden, blocks in the same group can
     * replace each other.
     */
    ...
    /**
     * The environment flags that this block can function in. If the env matches any of these, it
     * will be enabled.
     */
    public int envEnabled = Env.terrestrial;
    /**
     * The environment flags that this block *cannot* function in. If the env matches any of
     * these, it will be *disabled*.
     */
    ...
    /**
     * If true, this block attacks and is considered a turret in the indexer. Building must
     * implement Ranged.
     */
    ...
    /**
     * If true, this block deconstructs immediately. Instant deconstruction implies no resource
     * refund.
     */
    ...
    /**
     * Building that is created for this block. Initialized in init() via reflection. Set
     * manually if modded.
     */
    ...
    /**
     * Regions indexes from icons() that are rotated. If either of these is not -1, other regions
     * won't be rotated in ConstructBlocks.
     */
    ...
    public void drawShadow(Tile tile) {
        ...
        Draw.rect(variants == 0 ? customShadowRegion :
                variantShadowRegions[Mathf.randomSeed(tile.pos(), 0, Math.max(0,
                        variantShadowRegions.length - 1))], tile.drawx(), tile.drawy(),
                tile.build == null ? 0f : tile.build.drawrot());
        Draw.color();
    }
    ...
    public float percentSolid(int x, int y) {
        ...
        return tile.getLinkedTilesAs(this, tempTiles).sumf(other -> !other.floor().isLiquid ? 1f
                : 0f) / size / size;
    }
        ...
        if ((consumesPower || outputsPower) && hasPower && connectedPower) {
            ...
            if (tile != null) {
                PowerNode.getNodeLinks(tile, this, player.team(), other -> {
                    ...
                    node.drawLaser(x * tilesize + offset, y * tilesize + offset, other.x, other.y
                            , size, other.block.size);

                    Drawf.square(other.x, other.y, other.block.size * tilesize / 2f + 2f,
                            Pal.place);
                });
            }
        }
    ...
    public float sumAttribute(@Nullable Attribute attr, int x, int y) {
        ...
        return tile.getLinkedTilesAs(this, tempTiles).sumf(other -> !floating && other.floor().isDeep() ? 0 : other.floor().attributes.get(attr));
    }
    ...
    public void addLiquidBar(Liquid liq) {
        addBar("liquid-" + liq.name, entity -> !liq.unlockedNow() ? null :
                new Bar(() -> liq.localizedName, liq::barColor,
    ...
    public <T extends Building> void addLiquidBar(Func<T, Liquid> current) {
        addBar("liquid",
                entity -> new Bar(() -> current.get((T) entity) == null || entity.liquids.get(current.get((T) entity)) <= 0.001f ? Core.bundle.get("bar.liquid") : current.get((T) entity).localizedName, () -> current.get((T) entity) == null ? Color.clear : current.get((T) entity).barColor(), () -> current.get((T) entity) == null ? 0f : entity.liquids.get(current.get((T) entity)) / liquidCapacity));
    }

    public void setBars() {
        addBar("health",
                entity -> new Bar("stat.health", Pal.health, entity::healthf).blink(Color.white));

        if (consPower != null) {
            boolean buffered = consPower.buffered;
            float capacity = consPower.capacity;

            addBar("power", entity -> new Bar(() -> buffered ? Core.bundle.format("bar" +
                    ".poweramount", Float.isNaN(entity.power.status * capacity) ? "<ERROR>" :
                    UI.formatAmount((int) (entity.power.status * capacity))) : Core.bundle.get(
                            "bar.power"), () -> Pal.powerBar,
        ...
        if (hasItems && configurable) {
            addBar("items", entity -> new Bar(() -> Core.bundle.format("bar.items",
                    entity.items.total()), () -> Pal.items,
    ...
    public boolean canReplace(Block other) {
        ...
        return other.replaceable && (other != this || (rotate && quickRotate)) && this.group != BlockGroup.none && other.group == this.group && (size == other.size || (size >= other.size && ((subclass != null && subclass == other.subclass) || group.anyReplace)));
    }
    ...
    public boolean configSenseable() {
        return configurations.containsKey(Item.class) || configurations.containsKey(Liquid.class) || configurations.containsKey(UnlockableContent.class) || configurations.containsKey(Block.class) || configurations.containsKey(UnitType.class);
    }
    ...
    /**
     * Called when a new build plan is created in the player's queue. Blocks can maintain a
     * reference to this plan and add configs to it later.
     */
    ...
    public void drawPlanConfigCenter(BuildPlan plan, Object content, String region, boolean cross) {
        ...
        Color color = content instanceof Item i ? i.color : content instanceof Liquid l ?
                l.color : null;
        if (color == null) {
            return;
        }
    ...
    /**
     * Transforms the internal position of this config using the specified function, and return
     * the result.
     */
    ...
    public TextureRegion editorIcon() {
        return editorIcon == null ? (editorIcon = Core.atlas.find(name + "-icon-editor")) :
                editorIcon;
    }
    ...
    protected TextureRegion[] icons() {
        ...
        return teamRegion.found() && minfo.mod == null ? new TextureRegion[]{r,
                teamRegions[Team.sharded.id]} : new TextureRegion[]{r};
    }
    ...
    public TextureRegion[] variantRegions() {
        return variantRegions == null ? (variantRegions = new TextureRegion[]{fullIcon}) :
                variantRegions;
    }
    ...
    public Rect bounds(int x, int y, Rect rect) {
        return rect.setSize(size * tilesize).setCenter(x * tilesize + offset,
                y * tilesize + offset);
    }
    ...
    public boolean environmentBuildable() {
        return (state.rules.hiddenBuildItems.isEmpty() || !Structs.contains(requirements,
                i -> state.rules.hiddenBuildItems.contains(i.item)));
    }
    ...
    public <T extends Consume> T findConsumer(Boolf<Consume> filter) {
        return consumers.length == 0 ? (T) consumeBuilder.find(filter) :
            ...
            while (buildType == null && Block.class.isAssignableFrom(current)) {
                ...
                if (type != null) {
                    ...
                    Constructor<? extends Building> cons =
                            (Constructor<? extends Building>) type.getDeclaredConstructor(type.getDeclaringClass());
                    buildType = () -> {
                        try {
    ...
    public ItemStack[] researchRequirements() {
        ...
        for (int i = 0; i < out.length; i++) {
            int quantity =
                    Mathf.round(60 * researchCostMultiplier + Mathf.pow(requirements[i].amount,
                            1.11f) * 20 * researchCostMultiplier * researchCostMultipliers.get(requirements[i].item, 1f), 10);
    ...
    public void getDependencies(Cons<UnlockableContent> cons) {
        ...
        for (var c : consumeBuilder) {
            ...
            //TODO: requiring liquid dependencies is usually a bad idea, because there is no
            // reason to pump/produce something until you actually need it.
    ...
    public void init() {
        ...
        if (health == -1) {
            ...
            health = round ? Mathf.round(size * size * scaledHealth, 5) :
        ...
        optionalConsumers =
                consumeBuilder.select(consume -> consume.optional && !consume.ignore()).toArray(Consume.class);
        nonOptionalConsumers =
                consumeBuilder.select(consume -> !consume.optional && !consume.ignore()).toArray(Consume.class);
        updateConsumers =
                consumeBuilder.select(consume -> consume.update && !consume.ignore()).toArray(Consume.class);
    ...
    public void load() {
        ...
        for (Team team : Team.all) {
            teamRegions[team.id] = teamRegion.found() && team.hasPalette ?
                    Core.atlas.find(name + "-team-" + team.name, teamRegion) : teamRegion;
        }
    ...
    public void createIcons(MultiPacker packer) {
        ...
        if (outlineIcon) {
            AtlasRegion atlasRegion = (AtlasRegion) gen[outlinedIcon >= 0 ? Math.min(outlinedIcon
                    , gen.length - 1) : gen.length - 1];
        ...
        for (var region : toOutline) {
            if (region instanceof AtlasRegion atlas) {
                ...
                Pixmap outlined = Pixmaps.outline(Core.atlas.getPixmap(region), outlineColor,
                        outlineRadius);
        ...
        if (gen.length > 1) {
            ...
            editorBase = gen[0] == null ? Core.atlas.getPixmap(fullIcon) :
                    Core.atlas.getPixmap(gen[0]);
        }
